...
EnrollAdmin() {
    ...
    fabric-ca-client enroll -u https://admin:adminpw@localhost:7054 --caname ca.example.sust.edu --tls.certfiles ${PWD}/fabric-ca/tls-cert.pem
...
RegisterNewAdmin() {
    ...
    # fabric-ca-client register --id.name admin2 --id.affiliation sust.department1 --id.attrs 'hf.Revoker=true,admin=true:ecert'
    # fabric-ca-client register -d --id.name admin2 --id.affiliation sust.department1 --id.attrs '"hf.Registrar.Roles=peer,client"' --id.attrs hf.Revoker=true
...
EnrollPeer() {
    ...
    fabric-ca-client enroll \
        ...
        --csr.hosts peer0.example.sust.edu \
        --tls.certfiles ${PWD}/fabric-ca/tls-cert.pem
}
...
AddNewIdentity() {
    ...
    fabric-ca-client identity add user1 \
        --tls.certfiles ${PWD}/fabric-ca/tls-cert.pem \
        --json '{"secret": "user1pw", "type": "client", "affiliation": "sust", "max_enrollments": 1, "attrs": [{"name": "hf.Revoker", "value": "true"}]}'
...
ModifyIdentity() {
    ...
    fabric-ca-client identity modify user1 --affiliation startech
...
AffiliationOperations() {
    # Add
    fabric-ca-client affiliation add sust.dept1

    # Modify
    fabric-ca-client affiliation modify startech --name org3

    # Remove
    fabric-ca-client affiliation remove startech

    # Get All Affiliation
    fabric-ca-client affiliation list

    # Get All with specific affliliation
    fabric-ca-client affiliation list --affiliation startech.dept1
